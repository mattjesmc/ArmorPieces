...
| --- | --- | --- |
| the authoring check every piece is saved against (`check_part.py`: COPLANAR, unpainted faces, paint off the model, pixels outside the silhouette) | 0 | every shipped part, the packs' included, is clean - or `tools/gate/accepted_parts.json` carries a person's acceptance of that exact problem, and a waiver whose problem stopped occurring fails too |
...
| the one packet (`PartsSwitchPayload`) | 1 | the server's parts switch through the codec it was registered with: the default, mod parts off with an id and a tag, and a rule naming a pack the client does not have - arrives as written (`ShippedWireTest.theOnePacketTravels`, 2026-09-14) |
...
| effects | 2 | equip through `/item replace`, then `/armorpieces effects` and `/attribute … get`: a glider reported as contributing, and a fixture attribute granted and taken back (`effects`); then one worn part per effect TYPE (`effect-types`, 2026-09-14): heel_wings' jump number in gold and in netherite; circlet's Hero of the Village on the ENTITY with an emerald set and gone with a diamond; the fixture `gate:barehanded` (`if_wearer` over an attribute - the mod's own claws and head_fins ship in packs the world does not install) granted empty-handed, taken back when a sword is picked up with the piece standing still, and back again; cloak's 10% / 25% by material in the report, and twenty-four arrows in survival moving the wearer off a marked block. What is NOT here: a glider's durability, which needs a wearer that actually flies |
