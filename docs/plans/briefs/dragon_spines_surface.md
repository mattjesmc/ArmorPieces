# Brief: Dragon Spines — the surface pass

**This is a RETROFIT, not a build.** `armorpieces_dragon:dragon_spines` is finished: its geometry is correct, its
...
- **Do not repaint the master.** `dragon_spines.png` is finished and it is the silhouette everything else
...
**1. Open it.**
    ...
    armorpieces_open { piece: "armorpieces_dragon:dragon_spines" }

No pack paths — this piece exists and `armorpieces_open` finds it by name. If it answers
`no piece armorpieces_dragon:dragon_spines`, **stop and tell me**: this Blockbench window's pack list has not been given
...
|---|---|---|---|
| `spine1_0`, `spine2_0`, `spine3_0`, `spine4_0` | the four spines | `#26222c` | `#3a3444` on `up` |
...
The shoulder plate `base_0` stays master greyscale under its new `guard` mask.
