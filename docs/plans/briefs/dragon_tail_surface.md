# Brief: Dragon Tail — the surface pass

**This is a RETROFIT, not a build.** `armorpieces_dragon:dragon_tail` is finished: its geometry is correct, its
...
- **Do not repaint the master.** `dragon_tail.png` is finished and it is the silhouette everything else
...
**1. Open it.**
    ...
    armorpieces_open { piece: "armorpieces_dragon:dragon_tail" }

No pack paths — this piece exists and `armorpieces_open` finds it by name. If it answers
`no piece armorpieces_dragon:dragon_tail`, **stop and tell me**: this Blockbench window's pack list has not been given
...
|---|---|---|---|
| `tail1_0`, `tail2_0`, `tail3_0` | the three tail segments | `#26222c` | `#3a3444` on `up` |
...
**5. Paint the fitting mask.** Sheet `part_guard`, flat **140**:
    ...
    armorpieces_paint { sheet: "part_guard", faces: { "base_0.*": 140, "base_1.*": 140, "base_2.*": 140, "base_3.*": 140 } }
...
The four ring cubes stay master greyscale under their new `guard` mask.
