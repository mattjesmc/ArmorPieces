# Brief: Dragon Knuckles — the surface pass

**This is a RETROFIT, not a build.** `armorpieces_dragon:dragon_knuckles` is finished: its geometry is correct, its
...
- **Do not repaint the master.** `dragon_knuckles.png` is finished and it is the silhouette everything else
...
**1. Open it.**
    ...
    armorpieces_open { piece: "armorpieces_dragon:dragon_knuckles" }

No pack paths — this piece exists and `armorpieces_open` finds it by name. If it answers
`no piece armorpieces_dragon:dragon_knuckles`, **stop and tell me**: this Blockbench window's pack list has not been given
...
**4. Paint the static layer.** Sheet `part_static`, real colour:
    ...
    armorpieces_paint { sheet: "part_static",
                       faces: { "knob_lower_0.*": "#26222c",
                                 "knob_middle_0.*": "#26222c",
                                 "knob_upper_0.*": "#26222c",
                                 "knob_lower_0.up": "#3a3444",
                                 "knob_middle_0.up": "#3a3444",
                                 "knob_upper_0.up": "#3a3444" } }

| cubes | what it is | base | lit |
|---|---|---|---|
| `knob_lower_0`, `knob_middle_0`, `knob_upper_0` | the three wing knuckles | `#26222c` | `#3a3444` on `up` |
...
The backing plate `base_0` stays master greyscale under its new `guard` mask.
