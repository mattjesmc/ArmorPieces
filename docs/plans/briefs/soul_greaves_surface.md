# Brief: Soul Greaves — the surface pass

**This is a RETROFIT, not a build.** `armorpieces_nether:soul_greaves` is finished: its geometry is correct, its
...
- **Do not repaint the master.** `soul_greaves.png` is finished and it is the silhouette everything else
...
**1. Open it.**
    ...
    armorpieces_open { piece: "armorpieces_nether:soul_greaves" }

No pack paths — this piece exists and `armorpieces_open` finds it by name. If it answers
`no piece armorpieces_nether:soul_greaves`, **stop and tell me**: this Blockbench window's pack list has not been given
...
**4. Paint the static layer.** Sheet `part_static`, real colour:
    ...
    armorpieces_paint { sheet: "part_static",
                       faces: { "flame_out_0.*": "#2f8f9c",
                                 "flame_mid_0.*": "#2f8f9c",
                                 "flame_in_0.*": "#2f8f9c",
                                 "flame_out_0.up": "#5fe0ea",
                                 "flame_mid_0.up": "#5fe0ea",
                                 "flame_in_0.up": "#5fe0ea" } }

| cubes | what it is | base | lit |
|---|---|---|---|
| `flame_out_0`, `flame_mid_0`, `flame_in_0` | the three soul flames | `#2f8f9c` | `#5fe0ea` on `up` |
...
The shin plate `base_0` stays master greyscale under its new `guard` mask.
