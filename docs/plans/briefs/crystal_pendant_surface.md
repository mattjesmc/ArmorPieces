# Brief: Crystal Pendant — the surface pass

**This is a RETROFIT, not a build.** `armorpieces_dragon:crystal_pendant` is finished: its geometry is correct, its
...
- **Do not repaint the master.** `crystal_pendant.png` is finished and it is the silhouette everything else
...
**1. Open it.**
    ...
    armorpieces_open { piece: "armorpieces_dragon:crystal_pendant" }

No pack paths — this piece exists and `armorpieces_open` finds it by name. If it answers
`no piece armorpieces_dragon:crystal_pendant`, **stop and tell me**: this Blockbench window's pack list has not been given
...
This piece already carries `armorpieces:gemstone` and that stays. Leave it out of the call and `set_part` keeps it; its mask sheet is already painted and is not yours to change.
...
|---|---|---|---|
| `base_2` | the caged end crystal | `#e070e0` | `#f6c2f6` on `up` |
...
The collar bar `base_0`, the link `base_1` and the four cage bars `base_3`..`base_6` are metalwork and stay master greyscale - they are what answers the trim.
