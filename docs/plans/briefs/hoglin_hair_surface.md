# Brief: Hoglin Hair — the surface pass

**This is a RETROFIT, not a build.** `armorpieces_nether:hoglin_hair` is finished: its geometry is correct, its
...
- **Do not repaint the master.** `hoglin_hair.png` is finished and it is the silhouette everything else
...
**1. Open it.**
    ...
    armorpieces_open { piece: "armorpieces_nether:hoglin_hair" }

No pack paths — this piece exists and `armorpieces_open` finds it by name. If it answers
`no piece armorpieces_nether:hoglin_hair`, **stop and tell me**: this Blockbench window's pack list has not been given
...
**4. Paint the fitting mask.** Sheet `part_inlay`, flat **140**:
    ...
    armorpieces_paint { sheet: "part_inlay", faces: { "bristle_f2_0.*": 140, "bristle_f1_0.*": 140, "bristle_m_0.*": 140, "bristle_b1_0.*": 140, "bristle_b2_0.*": 140 } }
