# Brief: Dragon Mask — the surface pass

**This is a RETROFIT, not a build.** `armorpieces_dragon:dragon_mask` is finished: its geometry is correct, its
...
- **Do not repaint the master.** `dragon_mask.png` is finished and it is the silhouette everything else
...
**1. Open it.**
    ...
    armorpieces_open { piece: "armorpieces_dragon:dragon_mask" }

No pack paths — this piece exists and `armorpieces_open` finds it by name. If it answers
`no piece armorpieces_dragon:dragon_mask`, **stop and tell me**: this Blockbench window's pack list has not been given
...
|---|---|---|---|
| `base_0`, `snout1_0`, `snout2_0` | the faceplate and the snout | `#26222c` | `#3a3444` on `up` |
...
**5. Paint the fitting mask.** Sheet `part_gemstone`, flat **140**:
    ...
    armorpieces_paint { sheet: "part_gemstone", faces: { "ridge_l_0.*": 140, "ridge_r_0.*": 140 } }
