# Brief: Ghast Tendrils — the surface pass

**This is a RETROFIT, not a build.** `armorpieces_nether:ghast_tendrils` is finished: its geometry is correct, its
master is painted, its check is clean and it is saved in the pack. You are adding a fitting to it, and
...
- **Do not repaint the master.** `ghast_tendrils.png` is finished and it is the silhouette everything else
...
**1. Open it.**
    ...
    armorpieces_open { piece: "armorpieces_nether:ghast_tendrils" }

No pack paths — this piece exists and `armorpieces_open` finds it by name. If it answers
`no piece armorpieces_nether:ghast_tendrils`, **stop and tell me**: this Blockbench window's pack list has not been given
...
**4. Paint the fitting mask.** Sheet `part_inlay`, flat **140**:

    armorpieces_paint { sheet: "part_inlay", faces: { "tendril_f_up_0.*": 140, "tendril_f_lo_0.*": 140, "tendril_m_up_0.*": 140, "tendril_m_lo_0.*": 140, "tendril_b_up_0.*": 140, "tendril_b_lo_0.*": 140 } }
