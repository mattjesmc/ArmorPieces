# Brief: Dragon Scales — the surface pass

**This is a RETROFIT, not a build.** `armorpieces_dragon:dragon_scales` is finished: its geometry is correct, its
...
- **Do not repaint the master.** `dragon_scales.png` is finished and it is the silhouette everything else
...
**1. Open it.**
    ...
    armorpieces_open { piece: "armorpieces_dragon:dragon_scales" }

No pack paths — this piece exists and `armorpieces_open` finds it by name. If it answers
`no piece armorpieces_dragon:dragon_scales`, **stop and tell me**: this Blockbench window's pack list has not been given
...
|---|---|---|---|
| `plate1_0`, `plate2_0`, `plate3_0` | the three scale plates | `#26222c` | `#3a3444` on `up` |
...
**5. Paint the fitting mask.** Sheet `part_guard`, flat **140**:
    ...
    armorpieces_paint { sheet: "part_guard", faces: { "plate4_0.*": 140 } }
...
The retaining band `plate4_0` stays master greyscale under its new `guard` mask.
