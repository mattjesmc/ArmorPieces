# Brief: Dragon Crest — the surface pass

**This is a RETROFIT, not a build.** `armorpieces_dragon:dragon_crest` is finished: its geometry is correct, its
...
- **Do not repaint the master.** `dragon_crest.png` is finished and it is the silhouette everything else
...
**1. Open it.**
    ...
    armorpieces_open { piece: "armorpieces_dragon:dragon_crest" }

No pack paths — this piece exists and `armorpieces_open` finds it by name. If it answers
`no piece armorpieces_dragon:dragon_crest`, **stop and tell me**: this Blockbench window's pack list has not been given
...
**3. Create the sheets.** This must come before painting — it is what creates them:
    ...
    armorpieces_set_part { fittings: ["armorpieces:gemstone"], static: true }
...
|---|---|---|---|
| `horn1_0`, `horn2_0`, `horn3_0`, `horn4_0`, `horn5_0` | the five horns of the ridge | `#26222c` | `#3a3444` on `up` |
...
**5. Paint the fitting mask.** Sheet `part_gemstone`, flat **140**:

    armorpieces_paint { sheet: "part_gemstone", faces: { "horn3_1.*": 140 } }
