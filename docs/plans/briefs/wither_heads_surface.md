# Brief: Wither Heads — the surface pass

**This is a RETROFIT, not a build.** `armorpieces_nether:wither_heads` is finished: its geometry is correct, its
...
- **Do not repaint the master.** `wither_heads.png` is finished and it is the silhouette everything else
...
**1. Open it.**
    ...
    armorpieces_open { piece: "armorpieces_nether:wither_heads" }

No pack paths — this piece exists and `armorpieces_open` finds it by name. If it answers
`no piece armorpieces_nether:wither_heads`, **stop and tell me**: this Blockbench window's pack list has not been given
...
|---|---|---|---|
| `base_1`, `base_2`, `base_3` | the skull, its brow and its jaw | `#332f2b` | `#4d4841` on `up` |
...
The shoulder mount `base_0` stays master greyscale and keeps its `guard` mask.
