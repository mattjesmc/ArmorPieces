# Brief: Strider Hair — the surface pass

**This is a RETROFIT, not a build.** `armorpieces_nether:strider_hair` is finished: its geometry is correct, its
...
- **Do not repaint the master.** `strider_hair.png` is finished and it is the silhouette everything else
...
**1. Open it.**
    ...
    armorpieces_open { piece: "armorpieces_nether:strider_hair" }

No pack paths — this piece exists and `armorpieces_open` finds it by name. If it answers
`no piece armorpieces_nether:strider_hair`, **stop and tell me**: this Blockbench window's pack list has not been given
...
**4. Paint the fitting mask.** Sheet `part_inlay`, flat **140**:
    ...
    armorpieces_paint { sheet: "part_inlay", faces: { "tuft1_0.*": 140, "tuft2_0.*": 140, "tuft3_0.*": 140 } }
