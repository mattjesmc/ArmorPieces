# Brief: Dragon Wings — the surface pass

**This is a RETROFIT, not a build.** `armorpieces_dragon:dragon_wings` is finished: its geometry is correct, its
...
- **Do not repaint the master.** `dragon_wings.png` is finished and it is the silhouette everything else
...
**1. Open it.**
    ...
    armorpieces_open { piece: "armorpieces_dragon:dragon_wings" }

No pack paths — this piece exists and `armorpieces_open` finds it by name. If it answers
`no piece armorpieces_dragon:dragon_wings`, **stop and tell me**: this Blockbench window's pack list has not been given
...
|---|---|---|---|
| `base_1`, `base_2` | the two membrane panels | `#3b3046` | `#5a4a70` on `up` |
| `spar1_l_0`, `spar2_l_0`, `spar1_r_0`, `spar2_r_0` | the four spars, which are wing bone | `#26222c` | `#3a3444` on `up` |
...
**5. Paint the fitting mask.** Sheet `part_inlay`, flat **140**:
    ...
    armorpieces_paint { sheet: "part_inlay", faces: { "base_1.*": 140, "base_2.*": 140 } }
...
The root block `base_0` stays master greyscale - it is the harness.
