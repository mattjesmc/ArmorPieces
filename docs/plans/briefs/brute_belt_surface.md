# Brief: Brute Belt — the surface pass

**This is a RETROFIT, not a build.** `armorpieces_nether:brute_belt` is finished: its geometry is correct, its
...
- **Do not repaint the master.** `brute_belt.png` is finished and it is the silhouette everything else
...
**1. Open it.**
    ...
    armorpieces_open { piece: "armorpieces_nether:brute_belt" }

No pack paths — this piece exists and `armorpieces_open` finds it by name. If it answers
`no piece armorpieces_nether:brute_belt`, **stop and tell me**: this Blockbench window's pack list has not been given
...
This piece already carries `armorpieces:inlay` and that stays. Leave it out of the call and `set_part` keeps it; its mask sheet is already painted and is not yours to change.
...
|---|---|---|---|
| `base_0` | the leather belt band | `#5d4026` | `#7d5a38` on `up` |
...
The buckle plate `base_1` and the hanging tab `base_2` stay master greyscale, and the existing `inlay` mask is untouched.
