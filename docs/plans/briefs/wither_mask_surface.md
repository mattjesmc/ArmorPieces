# Brief: Wither Mask — the surface pass

**This is a RETROFIT, not a build.** `armorpieces_nether:wither_mask` is finished: its geometry is correct, its
...
- **Do not repaint the master.** `wither_mask.png` is finished and it is the silhouette everything else
...
**1. Open it.**
    ...
    armorpieces_open { piece: "armorpieces_nether:wither_mask" }

No pack paths — this piece exists and `armorpieces_open` finds it by name. If it answers
`no piece armorpieces_nether:wither_mask`, **stop and tell me**: this Blockbench window's pack list has not been given
...
|---|---|---|---|
| `base_0`, `base_2`, `base_3` | the faceplate, the jaw and the nose ridge | `#332f2b` | `#4d4841` on `up` |
...
**5. Paint the fitting mask.** Sheet `part_guard`, flat **140**:
    ...
    armorpieces_paint { sheet: "part_guard", faces: { "base_1.*": 140 } }
...
The brow ledge `base_1` stays master greyscale under its new `guard` mask.
