# Brief: Magma Cops — the surface pass

**This is a RETROFIT, not a build.** `armorpieces_nether:magma_cops` is finished: its geometry is correct, its
...
- **Do not repaint the master.** `magma_cops.png` is finished and it is the silhouette everything else
...
**1. Open it.**
    ...
    armorpieces_open { piece: "armorpieces_nether:magma_cops" }

No pack paths — this piece exists and `armorpieces_open` finds it by name. If it answers
`no piece armorpieces_nether:magma_cops`, **stop and tell me**: this Blockbench window's pack list has not been given
...
|---|---|---|---|
| `base_0` | the magma block - `up` takes the hot seam colour | `#d1440f` | `#ff8f30` on `up` |
...
**5. Paint the fitting mask.** Sheet `part_guard`, flat **140**:
    ...
    armorpieces_paint { sheet: "part_guard", faces: { "base_1.*": 140, "base_2.*": 140 } }
...
The boss `base_1` and the lip `base_2` stay master greyscale under their new `guard` mask.
