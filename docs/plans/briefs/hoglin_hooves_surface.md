# Brief: Hoglin Hooves — the surface pass

**This is a RETROFIT, not a build.** `armorpieces_nether:hoglin_hooves` is finished: its geometry is correct, its
...
- **Do not repaint the master.** `hoglin_hooves.png` is finished and it is the silhouette everything else
...
**1. Open it.**
    ...
    armorpieces_open { piece: "armorpieces_nether:hoglin_hooves" }

No pack paths — this piece exists and `armorpieces_open` finds it by name. If it answers
`no piece armorpieces_nether:hoglin_hooves`, **stop and tell me**: this Blockbench window's pack list has not been given
...
|---|---|---|---|
| `base_1`, `base_2` | the two cloven hooves | `#42342a` | `#63513f` on `up` |
...
The mount plate `base_0` stays master greyscale under its new `guard` mask.
